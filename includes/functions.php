<?php
     ...
     * Plugin Functions
     * @package   Lurma
     * @link      https://github.com/bahiirwa/Upgrade-for-Unattach-Re-Attach-Media-Attachments/
     */
     ...
     */
     ...
     function lurma_manage_upload_columns($columns) {
     	...
     	$columns['extended_parent'] = __( 'Parent', 'lurma');
     	return $columns;
     }
     ...
     function lurma_manage_media_custom_column($column_name, $id) {
     	...
     	if ($post->post_parent) {
     		...
     		?>
         <strong><a href="<?php echo get_edit_post_link( $post->post_parent ); ?>"><?php echo $title ?></a></strong>, <?php echo __('Uploaded on', 'lurma') . ' ' . get_the_time(__('d/m/Y')); ?><br />
         <a class="hide-if-no-js" onclick="findPosts.open('media[]','<?php echo $post->ID ?>'); return false;" href="#the-list"><?php _e('Re-Attach', 'lurma'); ?></a>
         <br />
         <a href="<?php echo esc_url( $url_unattach ); ?>" title="<?php echo __( "Unattach this media item.", 'lurma'); ?>"><?php _e( 'Unattach', 'lurma') ?></a>

        <?php
        } else {
          _e( '(Unattached)', 'lurma' ); ?>
         <br />
         <a class="hide-if-no-js" onclick="findPosts.open('media[]','<?php echo $post->ID ?>'); return false;" href="#the-list"><?php _e('Attach', 'lurma'); ?></a>
         <?php
     	  }
     }
     	...
     	if( is_admin() ) {
          ...
          <script type="text/javascript">
     				jQuery(document).ready(function() {
     					$ = jQuery;
     					$('<option>').val('unattach').text('<?php _e('Unattach', 'lurma')?>').appendTo("select[name='action']");
     					$('<option>').val('reattach').text('<?php _e('Re-Attach', 'lurma')?>').appendTo("select[name='action']");
     					$('<option>').val('unattach').text('<?php _e('Unattach', 'lurma')?>').appendTo("select[name='action2']");
     					$('<option>').val('reattach').text('<?php _e('Re-Attach', 'lurma')?>').appendTo("select[name='action2']");
     	...
     	if ( !isset( $_REQUEST['detached'] ) ) {
     		...
     		switch($action) {
     			case 'unattach':
     				...
     				if ( !is_admin() )
     					wp_die( __('You are not allowed to unattach this post.', 'lurma') );
     				...
     				foreach( $post_ids as $post_id ) {
     					...
     					if ( $wpdb->update($wpdb->posts, array('post_parent'=>0), array('id' => (int)$post_id, 'post_type' => 'attachment')) === false)
     						wp_die( __('Error unattaching post.', 'lurma') );
     					$unattached++;
     				}
